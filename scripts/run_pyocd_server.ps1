...
while ($true) {
    if ($args.Count -eq 0) {
        pyocd gdbserver --allow-remote --pack "$ScriptDir\..\packs\g4.pack" --pack "$ScriptDir\..\packs\h7.pack"
    }
    elseif ($args.Count -eq 1) {
        pyocd gdbserver --allow-remote --pack "$ScriptDir\..\packs\g4.pack" --pack "$ScriptDir\..\packs\h7.pack" $args[0]
    }
